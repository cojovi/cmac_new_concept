...
export const aiBody: Block[] = [
  ...
  { t: 'h2', text: 'Machine-readable indexes' },
  {
    ...
    items: [
      ...
      '/.well-known/ai-catalog.json — Agentic Resource Discovery entries for the API, MCP server, and research skill',
      '/.well-known/agent-skills/index.json — integrity-pinned discovery for the CMAC site research skill',
      '/agent?path=/services/roofing — one published page as metadata plus markdown JSON',
      '/ask — a GET or POST NLWeb 0.55 endpoint accepting a natural-language query and returning ranked JSON results',
      '/mcp — a public, read-only Streamable HTTP MCP server for site search, pages, services, locations, and contact details',
      '/.well-known/mcp — the same MCP transport at its discovery-first URL',
      '/.well-known/mcp/server-card.json — Model Context Protocol discovery',
      '/schemamap.xml — the NLWeb Schema Feeds index for the site-wide JSON Lines feed',
    ],
  },
