...
check(urls.length === 64, 'sitemap exposes exactly 64 public content routes', String(urls.length))
...
for (const path of ['/llms.txt', '/llms-full.txt', '/services/llms.txt', '/locations/llms.txt']) {
  const response = await request(path)
  check(response.status === 200, `${path} returns 200`, String(response.status))
  const text = await response.text()
